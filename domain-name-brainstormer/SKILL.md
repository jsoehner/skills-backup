...
| Avoid | Why | Instead |\\
|-------|-----|---------|\\
| Double letters on junction | e.g. `pressstart.com` leads to spelling errors | Keep boundaries clear |\\
| Typosquatting/Trademark conflicts | Legal violations, gets blacklisted | Verify trademark registry first |\\
| Numbers or hyphens | Hard to share verbally | Use letters-only brand names |\\

## DNS Availability Checks\\
To dynamically verify if a domain is resolved before suggesting it, run:\\
```bash\\
host -t a [domain-name]\\
```\\
If this returns an IP address, the domain is likely taken.\\
\\
## References\\
