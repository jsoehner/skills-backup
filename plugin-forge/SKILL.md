...
description: |
  ...
  Create and manage AI coding agent plugins with proper structure, manifests, and marketplace integration. Use when creating plugins for a marketplace, adding plugin components (commands, agents, hooks), bumping plugin versions, or working with plugin.json/marketplace.json manifests.
...
Build and manage AI coding agent plugins with correct structure, manifests, and marketplace integration. Includes workflows, automation scripts, and reference docs.
...
| Uppercase namespace folder names | Breaks CLI routing parameters | Use lowercase characters and `:` delimiters |

| Raw commits without checks | Syntax errors block local install workflows | Run validation checks on `plugin.json` schemas before committing |
...
- Before modifying marketplace properties, you **MUST** read [marketplace-schema.md](references/marketplace-schema.md) fully.

- Before restructuring directories or adding hooks, you **MUST** read [plugin-structure.md](references/plugin-structure.md).

- **Do NOT load** workflows reference guides if you are only running simple version bumps.
