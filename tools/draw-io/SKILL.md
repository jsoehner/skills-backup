...
| Avoid | Why | Instead |
|-------|-----|---------|
| Unclosed tags or mismatched keys | Corrupts the entire `.drawio` project file | Use XML syntax linters or check tag pairs before writing |
| Overlapping connectors | Makes readability difficult, confusing flow | Route lines around boxes; adjust coordinates |
| Custom raw group shapes | Breaks grouping properties, hard to move | Use official background frames |

## References & Loading Triggers

**MANDATORY - LOADING TRIGGERS**:
- Before searching for cloud infrastructure symbols or editing diagrams, you **MUST** read [aws-icons.md](references/aws-icons.md) or run the search helper script.
- Before modifying layouts, you **MUST** read [layout-guidelines.md](references/layout-guidelines.md) to inspect padding properties.
- **Do NOT load** references if you are merely running standard png output compilation commands (`convert-drawio-to-png.sh`).

## Image Display in reveal.js Slides
