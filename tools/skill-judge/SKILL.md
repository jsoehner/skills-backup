...
**For simple Skills** (no references, <100 lines): Score based on conciseness and self-containment. **MANDATORY**: For self-contained skills with no references, explicit instruction should state: *Do NOT load external files or reference directories*.
...
| 11-13 | Good calibration for most scenarios (minor guidance mismatches) |
| 14-15 | Perfect: clear rules for error scenarios vs flexible templates where judgment is value-add |
...
- High consequence → Low freedom (low tolerance for variance)
- Low consequence → High freedom (flexible parameters allowed)
- Medium consequence → Medium freedom (principles constraint but judgment is key)
...
### Grade Inflation Limits & Token Validation Commands
