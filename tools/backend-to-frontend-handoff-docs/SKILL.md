...
- Include real example payloads where helpful. **NEVER use truncated placeholder values** (e.g. `{"..."}`) in examples; map actual properties.\\
...
\\
### Pre-Handoff Checklist (Mindset check)\\
Before writing the handoff file, ask yourself:\\
1. Have I tested the endpoints locally to verify they match the exact shapes I'm documenting?\\
2. Did I explicitly double-check validation logic inside the controller/DTO?\\
\\
## References\\
\\
