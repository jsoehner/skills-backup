...
### Subagent Validation & Binary Image Checks
1. If subagents update Excalidraw JSON layouts, they **MUST** validate syntax correctness before writing (e.g. running a quick `python -m json.tool` parsing check).
2. If visual layouts require verification, subagents should convert the Excalidraw file to a static image format (e.g. PNG via CLI tools if available) rather than attempting to read raw coordinates.
