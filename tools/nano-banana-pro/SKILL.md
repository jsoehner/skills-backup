...
| Avoid | Why | Instead |
|-------|-----|---------|
| Omit Base64 Error catches | Output variables can write corrupted images if API buffers drop | Wrap base64 decoding loops in `try/except` blocks |
| Fixed aspect ratios for layouts | Mismatched dimensions break responsiveness on target devices | Derive output dimensions from device targets (e.g. mobile = `9:16`) |
