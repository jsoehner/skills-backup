...
- Before executing database normalizations or selecting schema layouts, you **MUST** read the deep-dives corresponding to your engine choice (e.g., `Deep Dive: Normalization` or `Deep Dive: NoSQL Design`).
- Before applying any structural migrations or schema upgrades, you **MUST** read `Deep Dive: Migrations` to inspect zero-downtime rules.
- If you are building a standard CRUD application with simple models, **do NOT load** the complete deep-dives to preserve context space.
