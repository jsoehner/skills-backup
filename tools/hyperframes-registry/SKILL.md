...
| Avoid | Why | Instead |
|-------|-----|---------|
| Registry commands for templates | Downloading raw slides directly via `add` fails | Use `init <dir> --example` |
| Mismatched Block IDs | Output timelines will fail to mount correctly | Ensure `data-composition-id` matches the source file's ID exactly |
| Raw component insertion | Pasting snippet templates without checking surrounding HTML syntax | Run `npx hyperframes lint` immediately after editing |
