...
| Deep imports (e.g. `@mui/material/Button`) | Fails in v7 since deep imports are removed | Use package exports index (e.g. `import { Button } from '@mui/material'`) |

| Hardcoded theme values | Breaks light/dark mode variations | Use theme design tokens (`primary.main`, `background.paper`) |

| Inconsistent spacing scales | Breaks design-system pixel alignments | Use theme spacing constants (`p: 2`, `mb: 3`) |
...
- Before customizing global themes or palette structures, you **MUST** read [theme-customization.md](resources/theme-customization.md) fully.

- Before setting up advanced CSS overrides, you **MUST** read [styling-guide.md](resources/styling-guide.md).

- **Do NOT load** components reference files if you are only implementing simple button variants.
