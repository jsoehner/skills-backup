...
Design/Code Change Proposal
 ├─ Low-risk, local, component-level, or reversible decision → [Use Minimum ADR Template]
 ├─ Default production app, cloud, security, data, or integration decision → [Use Standard ADR Template]
 ├─ Enterprise-critical, regulated, high-risk, or strategic decision → [Use Comprehensive ADR Template]
 └─ Local refactoring or minor bug fix within existing architecture → [No ADR Needed]
```
...
- If **Low Risk/Component-level**: Recommend drafting a new ADR using the `adr-authoring` skill with the **Minimum ADR Template**.
- If **Standard Production**: Recommend drafting a new ADR using the `adr-authoring` skill with the **Standard ADR Template**.
- If **Enterprise-critical / Regulated / High Risk**: Recommend drafting a new ADR using the `adr-authoring` skill with the **Comprehensive ADR Template**.
