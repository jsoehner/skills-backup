...
| Avoid | Why | Instead |
|-------|-----|---------|
| Exceeding bullet limits | Cramming >5 bullet points makes text unreadable | Split dense content across multiple slides |
| Default layouts for screenshots | Placing text over images without sizing coordinates | Use Marp split-image syntax `![bg right:40%](image.png)` |
| Omitting CSS variables | Custom presentation themes lose layout properties | Ensure CSS parameters are embedded in the markdown file |

## References & Loading Triggers

**MANDATORY - LOADING TRIGGERS**:
- Before organizing slides or writing markdown blocks, you **MUST** read [marp-syntax.md](references/marp-syntax.md) fully.
- Before embedding custom graphics or layout assets, you **MUST** read [image-patterns.md](references/image-patterns.md).
- Before designing corporate decks, you **MUST** read [theme-selection.md](references/theme-selection.md).
- **Do NOT load** custom CSS reference guides if you are using pre-packaged basic layouts.
