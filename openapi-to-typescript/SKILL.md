...
| Default `any` assignments | Bypasses TypeScript compile checking | Fallback to `unknown` and issue compiler warnings |

| Bypassing verification steps | Typo syntax errors can get written to production files | Run `tsc --noEmit` on generated files immediately after output |
