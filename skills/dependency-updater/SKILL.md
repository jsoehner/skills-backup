...
- **urllib Dynamic URLs**: When writing scripts that fetch metadata (e.g. querying Maven Central), **never** pass raw variables directly to `urllib.request.urlopen` or `urllib.request.Request`. Validate inputs with regex (e.g. `^[a-zA-Z0-9.\-_]+$`), verify the target URL starts with a trusted prefix (e.g. `https://repo1.maven.org/maven2/`), and use `# nosemgrep: urllib-ssrf-or-lfi` to satisfy Semgrep.

- **Git Rebasing & Diverged Repos**: Always fetch remote commits (`git fetch --all`) and check `git status` before beginning work. If the local workspace has diverged from remote (e.g. after PR merges), reset/rebase cleanly to avoid merge conflict loops.

- **Git Credential Retrieval**: If GitHub CLI (`gh`) fails with `gh auth login` required, retrieve the cached personal access token from Git Credential Manager by running:

  `echo "protocol=https`nhost=github.com" | git credential fill`

  and assign it to the `GH_TOKEN` or `GITHUB_TOKEN` environment variable.



### Verification Loop & Failures

After executing any minor or patch dependency upgrade, you **MUST** run the project's build command (e.g. `npm run build`, `mvn clean test`, or `pytest`) to verify compilation. If compilation or tests fail:

- Revert the dependency change immediately.

- Document the build error.

- Notify the user of the update regression.
...
- Before executing complex node updates, you **MUST** read `Deep Dive: Node.js with taze` and `Deep Dive: Conflict Resolution` sections.

- **Do NOT load** language specific deep-dives if you are merely executing a security audit (`npm audit`, `pip-audit`).
