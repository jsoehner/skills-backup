...
- Prompt: "Based on workflow type [--trunk-based or --feature-branch], prepare branch strategy. For feature branch: ensure branch name follows pattern (feature|bugfix|hotfix)/<ticket-id>-<description>. For trunk-based: prepare for direct main push with feature flag strategy if needed. Current branch: [insert branch], target: [insert target branch]. Verify no conflicts with target branch."
...
- Context from previous: All validation results
...
### Dependabot & Dependency Alert Management

- **Enabling Dependabot**: Configure Dependabot version and security updates in `.github/dependabot.yml`. Include the security-mandated `cooldown: default-days: 7` configuration for each package-ecosystem. Ignore major version updates for container base images (e.g., `eclipse-temurin` under `semver-major`) if the compile toolchain restricts upgrades.
- **Managing Alerts**: Check active alerts regularly using GitHub CLI. Dismiss resolved or mitigated alerts programmatically with:
  ```bash
  gh api -X PATCH repos/<owner>/<repo>/dependabot/alerts/<alert_number> \
    -f state="dismissed" \
    -f dismissed_reason="tolerable_risk" \
    -f dismissed_comment="Resolved by dependency updates/constraints in default branch"
  ```
