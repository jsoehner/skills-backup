...
| Vague indices (e.g., `data1`, `data2`) | Obscures the semantic differences between variables | Use descriptive scopes (`activeUsers`, `inactiveUsers`) |

| Unexplained abbreviations | Increases cognitive load for other developers | Spell out full terminology words (`calculateTotal`) |

| Modifying files without index updates | Breaks project compilation and search references | Use structural IDE rename configurations |
...
- Before executing search replacement refactoring scripts or renaming directories, you **MUST** run the name comparison mapping checks.

- **Do NOT load** language casing conventions tables if you are only verifying localized variables names scopes.
