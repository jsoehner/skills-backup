...
- Before starting Phase 2 code removal, you **MUST** read the target mindset file under [references/](references/) fully.\\
- Before committing codebase changes, you **MUST** read [adding-reference-mindsets.md](adding-reference-mindsets.md).\\
- **Do NOT load** mindsets reference files during the initial requirements check.\\
\\
### Git Diff Statistics & Test Safeguards\\
1. **Line count validation**: Prior to committing changes, run `git diff --stat` to verify a net reduction in overall lines of code.\\
2. **Testing safeguards**: When deleting code blocks, immediately execute the repository's test suite to ensure no active paths or logic dependencies are broken.\\
\\
## 6) Capture Knowledge
