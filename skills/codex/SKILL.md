...
description: |
  ...
  Use when the user asks to run Codex CLI (codex exec, codex resume) or references OpenAI Codex for code analysis, refactoring, or automated editing. Uses LLM.2 by default for state-of-the-art software engineering.
...
**LLM.2 Advantages**: 76.3% SWE-bench (vs 72.8% LLM), 30% faster on average tasks, better tool handling, reduced hallucinations, improved code quality. Knowledge cutoff: September 30, 2024.
...
## Error Handling & Sandbox Fallbacks
...
- **Sandbox Permission Failures**: If execution fails with a sandbox permission error, do not retry immediately; check the required access mode and suggest switching from `read-only` to `workspace-write` or request necessary scope approvals.
...
- **NEVER** run long or complex Codex executions without validating file existence or running basic syntax checks first.

- **NEVER** pass CLI flags when resuming a session unless the user explicitly requested them.

- **NEVER** omit `2>/dev/null` unless thinking tokens stderr outputs are specifically needed for debugging.
...
Requires Codex CLI v0.57.0 or later for LLM.2 model support. The CLI defaults to `gpt-5.2` on macOS/Linux and `gpt-5.2` on Windows. Check version: `codex --version`
