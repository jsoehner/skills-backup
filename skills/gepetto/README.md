...
**AI coding agent only** - This skill is designed specifically for AI coding agent.
...
| `claude-ralph-loop-prompt.md` | One-command execution with ralph-loop (AI coding agent plugin) |
| `claude-ralphy-prd.md` | One-command execution with Ralphy (external CLI) |

## External Review

gepetto uses **AI CLI** and **Codex CLI** to get independent reviews of your plan.
...
### Option B: Autonomous with ralph-loop (AI coding agent Plugin)

Best for: hands-off execution within AI coding agent, large plans, overnight runs.
...
Best for: multi-engine support (Claude, Codex, IDE, etc.), parallel execution, branch-per-task workflows.
...
```bash
# Via AI coding agent plugin marketplace
...
Ralphy is an external CLI tool that iterates through a task list (PRD.md) and executes each task using an AI CLI of your choice. Unlike ralph-loop (which runs inside AI coding agent), Ralphy runs externally and supports multiple AI engines.

### Key Differences: ralph-loop vs Ralphy

| Feature | ralph-loop | Ralphy |
|---------|-----------|--------|
| **Runs in** | AI coding agent (plugin) | External CLI |
| **AI Engines** | Claude only | Claude, Codex, IDE, Qwen, Droid |
