...
* **Discovery**: Clients can obtain a list of available tools by sending a `list` request
* **Invocation**: Tools are called using the `call` request, where servers perform the requested operation and return results
...
2. Servers can notify clients when tools change using `notificatio../list_changed`
