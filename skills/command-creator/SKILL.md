...
This skill guides the creation of AI coding agent slash commands - reusable workflows that can be invoked with `/command-name` in AI coding agent conversations.
...
- **MANDATORY**: Before prompt detail gathering or layout planning, you **MUST** read [patterns.md](references/patterns.md) and [best-practices.md](references/best-practices.md) fully to align formatting and logic.

- Load [examples.md](references/examples.md) when you need to copy specific structural boilerplate templates.

- **Do NOT load** external instructions or other references if designing a standard workflow with obvious commands.
...
| Duplicating System Rules | Bloats context window and creates conflicts | Only define custom workflows; let Agent default rules handle standard git/file/command behaviors |

| underscore_names | Breaker of consistency | Always use kebab-case (`my-command.md`) |
