...
| Hardcoding test accounts details | Security vulnerabilities leak credentials in logs | Use environment configuration flags |

| Sign-off without smoke validation | Minor commits can introduce breaking bugs | Run critical regression smoke tests before release sign-off |
...
- Before executing visual comparisons against UI layouts, you **MUST** read [figma_validation.md](references/figma_validation.md) fully.

- Before constructing test templates or bug tickets, you **MUST** read [test_case_templates.md](references/test_case_templates.md).

- **Do NOT load** regression guides during the initial planning phase to save context tokens.
