...
When configuring Dependabot, always consider security scanning policies and toolchain constraints:
1. **Add Cooldown Periods**: Use a `cooldown` block (`default-days: 7`) to prevent newly published packages from immediately triggering updates, satisfying security scan rules (like Semgrep) and avoiding unstable releases.
2. **Exclude Incompatible Toolchains**: Ignore major version updates for container base images (e.g. `eclipse-temurin`) when they are strictly constrained by the project's target toolchain (e.g., compile toolchain is Java 21 but Dependabot tries to bump the base image to Java 25).
