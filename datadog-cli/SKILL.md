...
| Avoid | Why | Instead |
|-------|-----|---------|
| Tailing logs with broad query | Hits API rate limits, returns noisy output | Always filter with scopes like `service:api status:error` |
| Metrics query without tags | Scans excessive metrics data, hits timeout limits | Filter with targeted tags like `{service:x}` |
| Querying wide timeframes | Exceeds fetch caps, slows down execution | Keep relative duration small (e.g. `--from 15m` or `1h`) |

## References & Loading Triggers

**MANDATORY - LOADING TRIGGERS**:
- Before running a timeseries metric query, you **MUST** read [metrics.md](references/metrics.md) and [query-syntax.md](references/query-syntax.md) fully to align queries.
- Before initiating any incident triage workflow, you **MUST** read [workflows.md](references/workflows.md) to inspect debugging rules.
- **Do NOT load** these references for simple diagnostic status check queries.
