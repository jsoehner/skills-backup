...
description: |
  "Interactive daily standup/meeting update generator. Use when user says 'daily', 'standup', 'scrum update', 'status update', 'what did I do yesterday', 'prepare for meeting', 'morning update', or 'team sync'. Pulls activity from GitHub, Jira, and AI coding agent session history. Conducts 4-question interview (yesterday, today, blockers, discussion topics) and generates formatted Markdown update."
...
│ • Check: AI coding agent history? gh CLI? jira CLI?     │
│ • AI coding agent → Pull yesterday's session digest     │
...
|-------------|-----------|
| **AI coding agent History** | `~/.claude/projects` directory exists with `.jsonl` files |
...
> **AI coding agent users:** Use `AskUserQuestionTool` tool for all questions in this phase.
...
### Step 4: Offer AI coding agent History

This integration captures everything you worked on with AI coding agent — useful for recalling work that isn't in git or Jira.
...
**If AI coding agent history exists, ask:**

```
"I can also pull your AI coding agent session history from yesterday. This can help recall work that isn't in git/Jira (research, debugging, planning). Want me to check?"

Options:
- "Yes, pull my AI coding agent sessions"
...
```
"Here are your AI coding agent sessions from yesterday. Select the ones relevant to your standup:"
...
- User explicitly says "No" to AI coding agent history
- User says they'll provide everything manually
- `~/.claude/projects` directory doesn't exist

**If digest script fails:**
- Fallback: Skip AI coding agent integration silently, proceed with interview
...
> **AI coding agent users:** Use `AskUserQuestionTool` tool to conduct the interview. This provides a better UX with structured options.
...
*AI coding agent only: Use `AskUserQuestionTool` tool for structured questions.

### AI coding agent Digest Script
...
Claude: "I can also pull your AI coding agent session history from yesterday. Want me to check?"
User: "Yes"

[Claude runs digest script, gets 5 sessions]

Claude: "Here are your AI coding agent sessions from yesterday. Select the ones relevant to your standup:"
...
- [From AI coding agent] Fix authentication timeout bug
- [From AI coding agent] Implement OAuth flow
- [From AI coding agent] Research payment providers
...
### Timezone & Formatting Tips
1. When checking history, verify the user's local system time to ensure "yesterday" matches the localized date offsets.
2. In UI multiSelect prompts, format option text clearly with project scopes (`[frontend]`, `[backend]`) so option lists are scannable on standard terminals.
